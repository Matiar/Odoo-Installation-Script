...
OE_OCA_HOME="$OE_HOME/custom/12.0"
...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/OCA/account-analytic.git $OE_OCA_HOME/bank-payment/
...
sudo pip3 install -r $OE_OCA_HOME/odoo-community-addons/requirements.txt
...
sudo pip3 install -r $OE_OCA_HOME/partner-contact/requirements.txt
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/OCA/purchase-workflow.git $OE_OCA_HOME/project/
...
sudo pip3 install -r $OE_OCA_HOME/server-tools/requirements.txt
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/OCA/server-tools.git $OE_OCA_HOME/server-ux/

sudo git clone --depth 1 --branch $OE_VERSION https://github.com/OCA/web.git $OE_OCA_HOME/web/

sudo pip3 install validate_email pymongo
